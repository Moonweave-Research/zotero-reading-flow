...
Version 1.3.6 has no separate analytics or reading-data upload service. It declares a GitHub update URL for retrieving release metadata and the release package; Zotero's own sync and update behavior remains Zotero-controlled.
...
Version 1.3.6 supports Zotero `9.0.x` and `10.0.x`, including compatible Zotero 9 source/self-built version strings. Its manifest uses `8.999` as Zotero's previous-major version-comparison sentinel for those Zotero 9 builds; Zotero 8 itself is not supported.
